...
import {selectBasketItems, selectBasketItemsWithId} from '../features/basketSlice';
...
const Dishrow = ({id,name, short_description, price, image}) => {
        ...
        const handleAddToBasket = ()=> {dispatch(addToBasket({id,name, short_description, price, image}))}
        ...
        {isPressed && 
           ( <View className="bg-white px-4">
                <View className="flex-row items-center space-x-2 p-1">
                    <TouchableOpacity disabled={!basketItems.length}  onPress={handleRemoveFromBasket} 
                    className={`bg-[#00CCBB]  ${!basketItems.length && "bg-gray-400"} rounded-full items-center p-1`}>
                        <Text className="text-xl">➖</Text>
                    </TouchableOpacity>
                    <Text className="text-xl">{basketItems.length}</Text>
                    <TouchableOpacity onPress={handleAddToBasket} className="bg-[#00CCBB] rounded-full items-center p-1">
                        <Text className="text-xl">➕</Text>
                    </TouchableOpacity>
                </View>
            </View>)}
